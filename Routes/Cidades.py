...
def Gerenciar():
    ...
    if request.method == 'POST':
        ...
        if 'arquivo_xlsx' in request.files:
            ...
            LogService.Info("Route.Cidades", f"Usuário {current_user.Nome} enviou arquivo de cidades: {Arquivo.filename}")
        ...
        # Confirmação do Modal
            ...
            LogService.Info("Route.Cidades", f"Usuário {current_user.Nome} confirmou substituição de cidades.")
...
def Excluir(id_remessa):
    LogService.Info("Route.Cidades", f"Usuário {current_user.Nome} solicitou exclusão da remessa {id_remessa}")
